...
factor1 = pd.read_excel("factor1-cp.xlsx",index_col="tradeDate")

# 去极值
lower_bound = factor1.quantile(0.25, axis=1,numeric_only=True)  # 每个横截面的下界，2%分位数
...
# 标准化
factor1_mean = factor1.nanmean(axis=1)  # 每个横截面的均值
factor1_std = factor1.nanstd(axis=1)  # 每个横截面的标准差
