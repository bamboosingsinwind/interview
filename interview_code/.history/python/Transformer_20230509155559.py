...
# PE(pos,2i)=sin(pos/10000^(2i/d_model))   i=1..d_model/2     PE(pos,2i+1)= cos(pos/10000^(2i/d_model))    
class PositionalEncoding(nn.Module):
    def __init__(self,d_model,dropout=0.1,maxlen=5000):
        super().__init__()
        self.d_model = d_model
        self.dropout = nn.Dropout(p=dropout)  # dropout rate 不一定要求d_model是偶
        self.maxlen = maxlen
    def forward(self,x):#[bs,seq_len,d_model]
        x = x[:self.maxlen]
        pos = torch.arange(0,self.d_model)
