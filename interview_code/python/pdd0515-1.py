...
def solve(s1,s2):
    ...
    def solve_sub(l1,l2):
        ...
        res = dis(l2[0],l1[0])
        sign = 1
        if abs(l1[0]-l2[0])!=res:sign=-1
        
        ll1 = [(i+sign*res)%16 for i in l1]#调整方式为取模
        #根据增益最大，确定转换子串位置。st:第一个不相等的或者直接取1也行；end：增益最大位置
        dis_gain = [dis(l1[i],l2[i])-dis(ll1[i],l2[i]) for i in range(n)]
        
        cum_gain = np.cumsum(dis_gain)
        end = np.argmax(cum_gain)
