from math import inf
#邻接表版(不含路径)======================
import heapq
from collections import defaultdict
nxs = defaultdict(list)
##input
'''
n = int(input())
for _ in range(n):#n: num of nodes
    u,v,w = [int(x) for x in input().split()]#u: source, v: destination, w: weight e.g
    nxs[u].append((v,w))# unidirectional
    # nxs[v].append((u,w))# bi-directional
'''
def dij(nxs,src,dst,n):
    dis = [inf for _ in range(n+1)]#node从1开始，不使用0号位
    dis[src] = 0
    q = [(0,src)] # (dis,node)
    while q:
        d,node = heapq.heappop(q)
        if node == dst:
            return d
        for nx,nxdis in nxs[node]:
            curdis = d + nxdis #dis(src,node) + dis(node,nx)
            if curdis < dis[nx]:
                dis[nx] = curdis
                heapq.heappush(q, (curdis, nx))

#邻矩阵版（不含路径）======================
'''
map = [[inf for _ in range(n+1)] for _ in range(n+1)]
for i in range(1,n+1):
    map[i][i] = 0
for _ in range(n):
    u,v,w = [int(i) for i in input().split(" ")]#map(int,input().split())
    map[u][v] = w
''' 
def dijkstra(map,src,dst):
    q = [(0,src)] # (dis,node)
    while q:
        d,node = heapq.heappop(q)
        if node == dst:
            return d
        for nx,nxdis in map[node][1:]:
            curdis = d + nxdis
            if curdis < map[src][nx]:
                map[src][nx] = curdis
                heapq.heappush(q, (curdis, nx))

#邻接矩阵版（不含路径）======================
def floyd(map):
    n = len(map)
    for k in range(n):
        for i in range(n): 	
            for j in range(n): 	
                if map[i][j] > map[i][k] + map[k][j]: 
                    map[i][j] = map[i][k] + map[k][j]
    return map

#邻接矩阵版（含路径）======================   
# https://blog.csdn.net/JohnJim0/article/details/109156986
def floyd(map):
    n = len(map)
    nx = [[-1 if map[i][j]== float("inf") else j for j in range(n)] for i in range(n)]#后继节点
    for k in range(n):
        for i in range(n): 	
            for j in range(n): 	
                if map[i][j] > map[i][k] + map[k][j]: 
                    map[i][j] = map[i][k] + map[k][j]
    return map,nx 